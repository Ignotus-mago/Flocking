...
// to make motion fit to torus

public class OpticalFlowMaker {
	...
	// TODO
	public OpticalFlowMaker(PApplet parent) {
		this(parent, 640, 480, 10, 1.0f);
	}

	public OpticalFlowMaker(PApplet parent, int wscreen, int hscreen, int gs, float predsec) {
		this.parent = parent;
	...
	// this is the one we're calling most reliably
	public OpticalFlowMaker(PApplet parent, int wscreen, int hscreen, int gs, float predsec, String cameraName) {
		this.parent = parent;
		this.wscreen = wscreen;
		this.hscreen = hscreen;
		this.gs = gs;
	...
	public void init(){
		...
		if ("".equals(cameraName)) {
			video = new Capture(parent, wscreen, hscreen, "Built-in iSight", fps);
			// video = new Capture(parent, devices[0]);
			parent.println("---- added camera iSight");
		}
		else {
	...
	public void flow() {
		if(video.available()){
			...
			responder.actionCallback(video);
...
*/
			...
			// here we can discover the vectors
			if(flagflow) {
				if (flagimage) parent.stroke(imageFlowColor);
				else parent.stroke(flowColor);
				parent.pushStyle();
				...
				for(int iy = 0; iy < gh; iy++) {
					...
					for(int ix = 0; ix < gw; ix++) {
						...
						float a = PApplet.sqrt(u * u + v * v);
						if(a >= 2.0) parent.line(x0, y0, x0 + u * flowScale, y0 + v * flowScale);
						// save the vectors
	...
	public void markGrid(int x, int y, int fillColor) {
		...
		parent.ellipse(x0, y0, gs, gs);
	}
	
	/**
	 * Returns vector at position (int, int) in the video 
	 * @param x
	 * @param y
	 * @return
	 */
	...
	public PVector getFlow(PVector vec) {
		return getFlow((int) vec.x, (int) vec.y);
	}
	...
	public void toggleImageDisplay() {
		flagimage = !flagimage;
		parent.println("flagimage = "+ flagimage);
	}
