...
/**
 * @author paulhz
 * updated for P5 version 2.2.5
 * 
 * Fourth version of flocking application. 
 ...
 * 
 */
public class Flocking03 extends PApplet {
	...
	enum boidPlacement {GRID, CENTER, RANDOM;}
	...
	public void setup() {
		// for an 1830 by 610 space
		size(1830, 610);
		// size((int)(1830 * 0.8f), (int)(610 * 0.8f));
		smooth();
		rando = new RandUtil();
		...
		sepFac = 97; alignFac = 89; cohFac = 123;
		useFactors = true;
		placement = boidPlacement.RANDOM;
	...
	public void addOneBoid(float x, float y) {
		...
		TurtleBoid tBoid = new TurtleBoid(this, new PVector(x, y), 3.0f, 0.05f);
	...
	public void saveAI(String filename) {
		PrintWriter pw = createWriter(filePath +"/"+ filename +"_"+ fileCount++ +".ai");
...
*/		
	...
	public void saveListAI(String filename) {
		filename = filePath +"/"+ filename +"_"+ fileCount++ +".ai";
...
//			if (tBoid instanceof TurtleBoid) println("it's a turtle boid");
			...
			if (!t.isEmpty()) {
				...
				if (neonEffect) {
					BezShape bezCopy = bez.clone();
					bezCopy.setWeight(bezCopy.weight() * 0.333333f);
					bezCopy.setStrokeColor(color(255));
